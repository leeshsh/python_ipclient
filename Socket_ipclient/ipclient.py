...
HOST = {
    'CONTROL PORT': 5200,
    'REFRESH PORT': 5201,
    'MAC ADDR'    : '2c:f0:ee:1a:76:68' # This is mac address just for test.
...
class Ipclient:
    def __init__(self):
        ...
        self.close_sock()
        userid = raw_input("请输入用户名:")
        passwd = raw_input("请输入密码:")
        self.init_sock(userid,passwd)
        res = self.login()
        print res[1]
        if res[0]:
            ...
            print get_news[1][0]
            while res[0]:
                refresh_online = self.refresh_online()
                print refresh_online[1][0],refresh_online[1][1]
                sleep(80)
        else:
            self.close_sock()
